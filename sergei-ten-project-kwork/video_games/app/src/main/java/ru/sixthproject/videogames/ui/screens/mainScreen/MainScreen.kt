...
import ru.sixthproject.videogames.data.firebase.videoGame.model.createFilmQuestionLoading
import ru.sixthproject.videogames.ui.view.SchoolBoard
import ru.sixthproject.videogames.yandexAds.InterstitialYandexAds
import ru.sixthproject.videogames.R
...
fun MainScreen(
    ...
    var film by remember { mutableStateOf(createFilmQuestionLoading()) }

    LaunchedEffect(key1 = Unit, block = {
        viewModel.getVideoGame(onSuccess = { film = it }, onFailure = { Log.e("onFailure",it) })
    })
                        ...
                        Column(
                            ...
                            modifier = Modifier
                                ...
                                .clickable {
                                    ...
                                    if (index == 1) {
                                        yandexAdsCount++
                                        userAnswer = film.year
                                    } else {
                                        if (userAnswer.lowercase() == film.year.lowercase()) {
                                            userAnswer = ""
                                            film = createFilmQuestionLoading()
                                            viewModel.getVideoGame(onSuccess = {
                                                film = it
                                            })
                                            Toast
                                                .makeText(context,
