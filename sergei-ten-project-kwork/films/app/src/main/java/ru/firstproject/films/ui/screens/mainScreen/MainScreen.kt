...
import ru.firstproject.films.data.firebase.film.model.createFilmQuestionLoading
import ru.firstproject.films.ui.view.SchoolBoard
import ru.firstproject.films.R
import ru.firstproject.films.yandexAds.InterstitialYandexAds
import ru.firstproject.films.yandexAds.RewardedYandexAds
import ru.firstproject.films.yandexAds.YandexAdsBanner
...
fun MainScreen(
    ...
    var rewarded by remember { mutableStateOf(true) }

    var userAnswer by rememberSaveable { mutableStateOf("") }
    var clickCount by rememberSaveable { mutableStateOf(0) }
    var yandexAdsCount by rememberSaveable { mutableStateOf(0) }
    var question by remember { mutableStateOf(createFilmQuestionLoading()) }

    LaunchedEffect(key1 = Unit, block = {
        viewModel.getFilmRandom(onSuccess = { question = it }, onFailure = { Log.e("onFailure",it) })
    })
                ...
                Column(
                    ...
                    modifier = Modifier
                        ...
                        .clickable {
                            ...
                            if (index == 1) {
                                yandexAdsCount++
                                userAnswer = question.year
                            } else {
                                if (userAnswer.lowercase() == question.year.lowercase()) {
                                    userAnswer = ""
                                    question = createFilmQuestionLoading()
                                    viewModel.getFilmRandom(onSuccess = {
                                        question = it
                                    })
                                    Toast
                                        .makeText(
